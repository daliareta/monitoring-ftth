...
export async function pollOlt(ip: string, community: string, type: 'ZTE' | 'HIOSO', snMac: string) {
  ...
  return new Promise<{ rx_live: number; tx_live: number; status: 'ONLINE' | 'LOS' | 'OFFLINE' }>((resolve, reject) => {
    try {
